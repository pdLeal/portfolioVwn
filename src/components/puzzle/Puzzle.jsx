...
function Puzzle() {
  ...
  const { instruction, hardCongrats, congrats, weakCongrats } = t("puzzleDescription");
        ...
        <S.Rules>
          {!projectWinner ? <S.TextRules>{instruction}</S.TextRules>
            : hardModeIsOn ? <S.TextRules>{hardCongrats}</S.TextRules>
              : <S.TextRules>{!isWeak ? congrats : weakCongrats}</S.TextRules>}

          <S.Btn className={ isWeak ? "disabled" : "" } $hardModeIsOn={hardModeIsOn} onClick={() => handleHardMode(handleRestart)}>{t("hardBtn")}</S.Btn>
